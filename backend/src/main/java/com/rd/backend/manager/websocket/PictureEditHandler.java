...
import com.rd.backend.manager.websocket.disruptor.PictureEditEventProducer;
import com.rd.backend.manager.websocket.model.PictureEditActionEnum;
import com.rd.backend.manager.websocket.model.PictureEditMessageTypeEnum;
import com.rd.backend.manager.websocket.model.PictureEditRequestMessage;
import com.rd.backend.manager.websocket.model.PictureEditResponseMessage;
...
public class PictureEditHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        User user = (User) session.getAttributes().get("user");
        Long pictureId = (Long) session.getAttributes().get("pictureId");
        // 根据消息类型处理信息（生产消息到 Disruptor 队列中）
    ...
    public void handleEditActionMessage(PictureEditRequestMessage pictureEditRequestMessage, WebSocketSession session, User user, Long pictureId) throws IOException {
        ...
        if (editingUserId != null && editingUserId.equals(user.getId())) {
            ...
            String message = String.format("%s 执行了 %s", user.getUserName(), pictureEditActionEnum.getText());
            pictureEditResponseMessage.setMessage(message);
            pictureEditResponseMessage.setEditAction(editAction);
            pictureEditResponseMessage.setUser(userService.getUserVO(user));
            // 广播给除了当前用户以外的其他用户，否则会出现重复编辑
